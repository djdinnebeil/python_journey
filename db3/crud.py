...
def get_products_paginated(page: int = 1, per_page: int = 3):
    """
    Retrieve paginated order items, including product names, efficiently.
    ...
    Returns:
        List[dict]: A list of order item dictionaries for the given page.
    """
    ...
    with db.session_scope() as session:
        stmt = (
            select(OrderItem)
            .options(selectinload(OrderItem.product))  # Eagerly load the product relationship
