...
db_connection = DatabaseConnection('db_simplify.db')
...
cursor.executemany('INSERT INTO companies (id, ticker, name, company_type) VALUES (?, ?, ?, ?)', foreign_companies)
...
class Company:
    def __init__(self, company_id, ticker, name, company_type):
    ...
    def get_company(identifier, conn):
        ...
        if isinstance(identifier, str):
            ...
            row = cursor.fetchone()
            if row:
                return Company(row[0], row[1], row[2], row[3])
        else:
...
# Example usage:
conn = sqlite3.connect('db_simplify.db')
...
try:
    domestic_company = Company.get_company('AAPL', conn)
    if domestic_company:
...
try:
    foreign_company = Company.get_company(1001, conn)
    if foreign_company:
